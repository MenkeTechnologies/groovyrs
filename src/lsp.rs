...
const CORPUS: &[(&str, &str, &str, &str, &str)] = &[
    ...
    ("1.50", "Literals and Literal Forms", "<digits>.<digits> [g|G]",
     ...
     "println(1.50)   // => 1.50"),
    ("1.5d", "Literals and Literal Forms", "<digits>.<digits> d|D",
     "A `d`-suffixed literal — an IEEE double, `java.lang.Double`. With the `f` form below, one of the two literal shapes that is *not* a `BigDecimal`, and the only one that reaches fusevm's native floating ops.",
     "println(1.5d / 0.0d)   // => Infinity"),
    ("1.5f", "Literals and Literal Forms", "<digits>.<digits> f|F",
     "An `f`-suffixed literal — an IEEE single, `java.lang.Float`. A different type from `Double`: it renders through `Float.toString` (24 significant bits) and hashes `floatToIntBits`. It has no arithmetic of its own — Groovy widens a `Float` to a `double` before every operator runs, so `0.1f + 0.1f` is the *Double* `0.20000000298023224` rather than `0.2`.",
     "println(0.1f + 0.1f)   // => 0.20000000298023224"),
    ("2.5e7", "Literals and Literal Forms", "<digits>[.<digits>]e[+|-]<digits>",
     "An exponent literal. The exponent makes the literal a decimal with or without a fractional part, and the result is a `BigDecimal` unless a `d` (`Double`) or `f` (`Float`) suffix follows.",
    ...
    ("subList", "GDK — List", "list.subList(from, to) -> List",
    ...
    ("addAll", "GDK — List", "list.addAll(collection) | list.addAll(index, collection) -> Boolean",
    ...
    ("Float", "Type Names", "value instanceof Float",
     "True for a `java.lang.Float` — an `f`-suffixed literal, `as Float`, `floatValue()`, or the `float` overload `java.lang.Math` resolves to. `Float` and `Double` are siblings, so neither is `instanceof` the other.",
     "println(1.5f instanceof Float)   // => true"),
    ("BigInteger", "Type Names", "value instanceof BigInteger",
     "True for a `java.math.BigInteger` — a `G`-suffixed literal, an integer literal past `Long`, or an overflowing integer `**`. A distinct type from `BigDecimal`, so a decimal answers false.",
     "println(5G instanceof BigInteger)   // => true"),
