...
const CORPUS: &[(&str, &str, &str, &str, &str)] = &[
    ...
    ("42", "Literals and Literal Forms", "<digits>",
     "An integer literal — a 64-bit `Value::Int`. A literal that does not fit is a lex error, not a silent wrap.",
     "println(42 + 1)   // => 43"),
    ("42L", "Literals and Literal Forms", "<digits>L | <digits>l",
     "A long-suffixed integer literal. groovyrs holds every integer as an i64, so the suffix is accepted and consumed but does not change the value's type.",
     "println(42L)   // => 42"),
    ...
    ("..", "Operators", "a..b",
     "An inclusive range. groovyrs materialises a range to a plain list at lowering time, so `(0..3).class.simpleName` is `ArrayList` where Groovy answers `IntRange` — and printing one shows `[0, 1, 2, 3]` where Groovy shows `0..3`.",
     "println(0..3)   // => [0, 1, 2, 3]"),
    ("..<", "Operators", "a..<b",
     "A half-open range: the endpoint is decremented before the range is materialised.",
     "println(0..<3)   // => [0, 1, 2]"),
    ("++", "Operators", "x++ | ++x",
    ...
    ("any", "GDK — Map Closure Methods", "map.any { k, v -> … } -> Boolean",
     "True when the closure accepts at least one entry; stops at the first acceptance. Map-only: Apache Groovy also defines `any` on a list, and groovyrs does not — a list receiver raises `MissingMethodException`.",
     "println([a: 1, b: 2].any { k, v -> v > 1 })   // => true"),
    ("every", "GDK — Map Closure Methods", "map.every { k, v -> … } -> Boolean",
     "True when the closure accepts every entry; stops at the first rejection. Map-only, like `any`, and missing from lists for the same reason.",
    ...
    ("List", "Type Names", "value instanceof List",
     "True for a list value — including a materialised range, since a range is a list.",
